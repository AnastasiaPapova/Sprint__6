...
class TestYandexScooterOrderPage:
    ...
    @allure.title("Оформление заказа по кнопке 'Заказать' в header на главной странице.")
    ...
    def test_header_button_create_order_input_correct_data_and_show_order_banner(self, driver, data_set):
        ...
        order_completed = order_page.order_completed_banner_is_displayed()
        assert order_completed

    @allure.title("Оформление заказа по кнопке 'Заказать' в блоке 'Как это работает'.")
    ...
    @allure.title("Заполнение данных для заказа и переход с шага 'Для кого самокат' на шаг 'Про аренду'.")
    ...
    def test_order_page_go_to_choose_scooter_user_data_correct_open_about_rent(self, driver):
        ...
        order_page.go_next()
        about_rent_step = order_page.check_about_rent_step_is_displayed()
        assert about_rent_step

    @allure.title("Проверка сообщений некорректно заполненных полей на форме офрмления заказа ")
    ...
    def test_order_page_input_incorrect_data_show_error_message(self, driver):
        ...
        order_page.go_next()
        incorrect_firs_name_message = order_page.check_incorrect_first_name_message()
        incorrect_last_name_message = order_page.check_incorrect_last_name_message()
        incorrect_adress_message = order_page.check_incorrect_adress_message()
        incorrect_tel_number_message = order_page.check_incorrect_tel_number_message()

        assert incorrect_firs_name_message
        assert incorrect_last_name_message
        assert incorrect_adress_message
        assert incorrect_tel_number_message

    @allure.title("Проверка сообщения об ошибке при не заполненном поле 'станция метро' ")
    @allure.description("Проверка что при пустом поле 'станция метро' в форме оформление заказа, выводится ошибка")
    def test_order_page_metro_station_input_is_empty_show_error_message(self, driver):
        ...
        main_page.click_cookie_accept_button()
        order_page.go_next()
        incorrect_metro_station_message = order_page.check_incorrect_metro_station_message()
        assert incorrect_metro_station_message
